 ...
 *
 */
function drawPlots(loanName, plotlyPmtsInputs, plotlyTotalsInputs) {
  ...
  Plotly.newPlot(paymentsGraphID, plotlyPmtsInputs, paymentsLayout);
  ...
  Plotly.newPlot(totalsGraphID, plotlyTotalsInputs, lifetimeLayout);
}
...
function renderUI() {
  ...
  paymentInputs.each(function(index) {
    ...
    tabulationValues["All_Loans"][pmtAsStr].interest =
      pmtSchedules["All_Loans"].lifetimeData.lifetimeInterestPaid;
    tabulationValues["All_Loans"][pmtAsStr].finalPaymentDate =
      pmtSchedules["All_Loans"].lifetimeData.finalPaymentDate;
    ...
    for (let loan of loansArray) {
      ...
      tabulationValues[loan.name][pmtAsStr].interest =
        pmtSchedules[loan.name].lifetimeData.lifetimeInterestPaid;
      tabulationValues[loan.name][pmtAsStr].finalPaymentDate =
        pmtSchedules[loan.name].lifetimeData.finalPaymentDate;
    }
  });
  ...
  tabulateLifetimeTotals("All_Loans", tabulationValues["All_Loans"], payments);

  for (const loan of loansArray) {
    tabulateLifetimeTotals(loan.name, tabulationValues[loan.name], payments);
  }
